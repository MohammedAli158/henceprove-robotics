...
export default function CardSection({ card }: { card: Card }) { 
  ...
  return (
    <section className="flex flex-col justify-center items-center my-8 w-full">
      {card.map((c, i) => {
        return (
          <div
            key={i}
            className={
              " w-6xl rounded-lg p-6 m-4 hover:shadow-md transition-shadow duration-300 flex flex-col md:flex-row gap-6  md:items-center " +
              bg[i % bg.length]
            }
          >
            
            <div className="flex flex-col md:w-1/2 w-full ">
              <h2 className="text-3xl md:text-4xl font-bold mb-4 text-center">{c.title}</h2>
